...
`agent-mesh daemon` 长期持有 Ed25519 身份和 libp2p Swarm。控制地址与随机令牌写入权限为 0600 的 `control.json`。本机 RPC 支持 `status`、`peers`、`publish`、`fetch`、`send`、`inbox.list` 和 `inbox.ack`。

P2P 信封分为 `message`、`task`、`task_cancel`、`task_progress`、`task_result`。信封包含网络、发送/接收 Peer ID、UUID、关联 UUID、创建/过期时间和 JSON payload。接收端验证网络、传输层 Peer ID、目标、TTL 与 allowlist，持久化成功后才返回 ACK；重复 UUID 幂等。节点声明可带 `dsh-leader/1` 协议、聚合角色、逻辑 workspace、本地 Team 能力与并发上限；teammate 从不作为 Mesh 成员发布。

### CLI

daemon 管理命令和日常操作位于同一 binary。`status/peers/publish/get/message/task/reply/inbox/ack` 读取控制文件连接 daemon。早期 `serve/discover` 保留为诊断兼容入口，日常运行应使用 daemon。
...
- Agent 信封默认拒绝，必须显式 allowlist；`network_id` 只是命名空间。
...
- 双 daemon 实测覆盖 bootstrap、Leader 协议声明、私网路由、RTT、消息/任务/取消落盘 ACK 和内容传输。
- Harness 真实 profile 安装、Loader 加载、托管 daemon 连接以及 Harness 退出后的进程树回收均已执行。

## 后续阶段

生产化的下一顺序应是：持久执行 lease/幂等结果缓存 → Leader task progress 与恢复 → 断点续传与并行窗口 → 对象 ACL/配额/审计 → AutoNAT/DCUtR/Relay → US3/S3 adapter 与账单对账 → 组织证书、信誉和结算。Agent Team 永远留在节点内部；跨节点只稳定 Leader 联邦协议，不暴露 Harness 内部 Agent 对象。
